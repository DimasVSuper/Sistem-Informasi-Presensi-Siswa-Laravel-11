...
| Komponen | Deskripsi |
|:---|:---|
| Pendahuluan | Menjelaskan latar belakang dan tujuan dibuatnya rencana pengujian. |
| Tujuan Pengujian | Menentukan sasaran pengujian, misalnya memastikan kualitas sistem presensi PWA. |
| Scope | Menjelaskan ruang lingkup pengujian, baik fungsional maupun non-fungsional. |
| Schedule | Pengujian dilakukan selama 2 minggu dengan pembagian tugas harian. |
| Resources | Tim terdiri dari 2 penguji, 1 manajer proyek, dan dukungan infrastruktur. |
| Environment | Server uji, database MySQL, API notifikasi, serta perangkat mobile untuk simulasi. |
| Functional Testing | Meliputi login, presensi QR, validasi identitas, notifikasi orang tua, dan laporan kehadiran. |
| UI/UX Testing | Menilai kemudahan penggunaan antarmuka oleh siswa, guru, dan orang tua. |
| Performance Testing | Mengukur waktu respon sistem saat presensi dilakukan (< 3 detik). |
| Security Testing | Menilai keamanan aplikasi, validasi input, proteksi akses, dan integritas data. |
| Defect Management | Proses pencatatan bug, pelaporan ke tim pengembang, dan tindak lanjut perbaikan. |
| Testing Tools | PHPUnit, Mockery, Postman, dan JMeter untuk uji performa. |
| Risk Management | Identifikasi risiko seperti downtime server atau kegagalan notifikasi, serta strategi mitigasi. |
| Exit Criteria | Menentukan kriteria keberhasilan pengujian, misalnya semua kasus uji kritis lulus tanpa bug mayor. |
