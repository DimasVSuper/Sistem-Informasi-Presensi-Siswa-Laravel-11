            ...
            <div class="mb-8">
                ...
                <select name="orang_tua_id" id="orang_tua_id" required 
                    ...
                    @foreach($orangTua as $parent)
                        <option value="{{ $parent->id }}" {{ old('orang_tua_id') == $parent->id ? 'selected' : '' }}>
                            {{ $parent->nama }} ({{ $parent->email }})
                        </option>
                    @endforeach
                </select>
                <p class="text-xs text-gray-500 mt-1">Siswa akan ditautkan ke orang tua untuk notifikasi email presensi.</p>
                @error('orang_tua_id')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>

            <div class="bg-gray-50 p-4 rounded-xl border border-gray-100 flex items-start mb-8">
                <i data-feather="info" class="w-5 h-5 text-indigo-500 mr-3 mt-0.5 border-indigo-200"></i>
                <p class="text-sm text-gray-600">QR Code untuk absensi akan di-generate secara otomatis oleh sistem setelah data siswa disimpan.</p>
            </div>
